...
test_that("Missing values", {
  ...
  df_clean_row <- remove_NA(as.data.frame(mtx), margin = 1, finite = FALSE)
  expect_equal(dim(clean_row), dim(df_clean_row))

  clean_col <- remove_NA(mtx, margin = 2, finite = FALSE)
  expect_lt(ncol(clean_col), ncol(mtx))

  df_clean_col <- remove_NA(as.data.frame(mtx), margin = 2, finite = FALSE)
  expect_equal(dim(clean_col), dim(df_clean_col))
...
test_that("Missing and non-finite values", {
  ...
  df_clean_row <- remove_NA(as.data.frame(mtx), margin = 1, finite = TRUE)
  expect_equal(dim(clean_row), dim(df_clean_row))

  clean_col <- remove_NA(mtx, margin = 2, finite = TRUE)
  expect_lt(ncol(clean_col), ncol(mtx))

  df_clean_col <- remove_NA(as.data.frame(mtx), margin = 2, finite = TRUE)
  expect_equal(dim(clean_col), dim(df_clean_col))
...
test_that("Zeros", {
  ...
  df_clean_row <- remove_zero(as.data.frame(mtx), margin = 1)
  expect_equal(dim(clean_row), dim(df_clean_row))

  clean_col <- remove_zero(mtx, margin = 2)
  expect_lt(ncol(clean_col), ncol(mtx))

  df_clean_col <- remove_zero(as.data.frame(mtx), margin = 2)
  expect_equal(dim(clean_col), dim(df_clean_col))
