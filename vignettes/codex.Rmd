...
# Definitions
The **codex** package provides a set of S4 classes for archaeological data matrices that extend the basic `matrix` data type. These new classes represent different special types of matrix.

* Abundance matrix:
  * `AbsoluteFrequencyMatrix` represents absolute frequency data (count).
  * `RelativeFrequencyMatrix` represents relative frequency data.
...
The internal structure of S4 classes implemented in **codex** is depicted in the UML class diagram in the following figure.
...
Note that a `RelativeFrequencyMatrix` can only be created by coercion (see below).
